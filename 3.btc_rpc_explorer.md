...
> node ./bin/www
  ...
  btcexp:app Config file not found at /home/pi/.config/btc-rpc-explorer.env, continuing... +1ms
  btcexp:app Config file not found at /etc/btc-rpc-explorer/.env, continuing... +0ms
  btcexp:app Config file found at /home/pi/btc-rpc-explorer/.env, loading... +0ms
  btcexp:app Default cacheId '3.5.1' +986ms
  btcexp:app Enabling view caching (performance will be improved but template edits will not be reflected) +20ms
  btcexp:app Session config: {"secret":"*****","resave":false,"saveUninitialized":true,"cookie":{"secure":false}} +16ms
  btcexp:app Enabling rate limiting: 200 requests per 15min +1ms
node:events:497
...
this means port 3002 is already in use, you can either stop the other service or change the port in `.env` file

```bash
$ sudo lsof -i :3002
COMMAND   PID USER   FD   TYPE DEVICE SIZE/OFF NODE NAME
node    <PID> 2345   pi   20u  IPv4   123456   0t0  TCP *:3002 (LISTEN)
```
